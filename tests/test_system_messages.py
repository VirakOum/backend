...
def test_system_messages_crud_and_active_flow():
    ...
    resp = client.post("/travel/admin/messages", json=create_payload)
    ...
    toggle_resp = client.post(f"/travel/admin/messages/{msg_id}/toggle-active")
    assert toggle_resp.status_code == 200
    assert toggle_resp.json()["is_active"] is False

    # Verify deactivated message no longer returns in active messages
    active_resp2 = client.get("/travel/messages/active?role=driver")
    ...
    del_resp = client.delete(f"/travel/admin/messages/{msg_id}")
